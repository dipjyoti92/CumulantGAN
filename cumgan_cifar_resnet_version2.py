...
def Discriminator(inputs, dim=DIM):
    ...
    output = ResidualBlock('Discriminator.Res2', dim, dim, 3, output, resample=None)             
    output = ResidualBlock('Discriminator.Res3', dim, dim, 3, output, resample=None)              

    output = tf.reshape(output, [-1, 4*8*8*dim])						   
    output = lib.ops.linear.Linear('Discriminator.Output', 4*8*8*dim, 1, output)                  
...
if MODE == 'wgan':
    ...
    gen_train_op = tf.train.AdamOptimizer(learning_rate=2e-4, beta1=0., beta2=0.9).minimize(gen_cost, var_list=gen_params)
    disc_train_op = tf.train.AdamOptimizer(learning_rate=2e-4, beta1=0., beta2=0.9).minimize(disc_cost, var_list=disc_params)
